...
- **סגירה אוטומטית של הצבעה** — `proposal_effective_status()` מחשב את מצב ההצעה
  ...
  ורשימת המצביעים נפתחת באותו רגע.
...
נכון ללא תלות במה שהממשק בוחר להציג. 58 בדיקות: שיוך תפקידים לפי קוד, חסימת
פעולות ועד, בידוד מלא בין בניינים, אנונימיות של מציעים ומצביעים, מניעת הצבעה
כפולה, ואפס גישה למשתמש לא מחובר.
...
- אין עריכה או מחיקה של תנועה בתקציב. הדרך הנכונה לתקן טעות היא תנועת תיקון
  נגדית, שמשאירה מסלול ביקורת — אבל זה לא נבנה.
- אין סגירה ידנית מוקדמת של הצעה; ההצבעה נסגרת לפי `closes_at` בלבד.
- אין התראות (מייל/פוש) על תקלה חדשה או על הצעה שנסגרת.
